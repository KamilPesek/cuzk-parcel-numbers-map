...
2. Create `config/config.php` and set `API_KEY` in it. See [config/config.php.dist](config/config.php.dist) for an example.
3. Provide input file. Input data are loaded from `config/parcel_numbers.json`. See [config/parcel_numbers.json](config/parcel_numbers.json.dist) for an example.
